...
// GET /payments - Vendor Payment Tracker Dashboard (Member B)
router.get('/', isAuthenticated, async (req, res) => {
    try {
        // Query financial summary for all vendors: Total PO Billed vs Total Paid
        let vendorQuery = `
        ...
        // If logged in user is a Vendor, restrict to their own vendor account
        ...
        `;
        if (req.session.user.role === 'Vendor') {
            txQuery += ` WHERE v.user_id = ${db.escape(req.session.user.id)} `;
        }
        txQuery += ` ORDER BY p.payment_date DESC, p.created_at DESC LIMIT 20`;
        const [recentPayments] = await db.query(txQuery);
...
// GET /payments/create - Record New Payment Form
...
// POST /payments/create - Record Payment Action
...
// GET /payments/vendor/:id - Detailed Vendor Ledger Account Statement
router.get('/vendor/:id', isAuthenticated, async (req, res) => {
    try {
        const vendorId = req.params.id;

        // Security check for vendor users
        if (req.session.user.role === 'Vendor') {
