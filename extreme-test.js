...
async function request(method, path, data = null, isMultipart = false, boundary = '----SmartConstructionBoundary12345') {
    return new Promise((resolve, reject) => {
        const url = new URL(path, baseUrl);
        const options = {
            method: method,
            headers: {
                'User-Agent': 'SmartConstruction-Extreme-QA-Auditor/2.0'
            }
        };
...
async function runExtremeAudit() {
    ...
    try {
        ...
        const boundary = '----SmartConstructionExtremeTestBoundary998877';
        const fileContent = 'This is a valid test contract agreement document generated by SmartConstruction Extreme QA.';
        const contractPayload = createMultipartPayload({ vendor_id: '1', title: '2026 Master Steel Agreement (Extreme Test)' }, 'contract_file', 'extreme_test_contract.pdf', fileContent, boundary);
